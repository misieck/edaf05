...
int calc_gain_candidate(size_t x, size_t y, str_data* str_in, in_data* input, int gains[str_in->len_x + 1][str_in->len_y + 1], direction weight_dir){
    if(!test_coordinate(str_in->len_x,str_in->len_y,x,y)) return INT_MIN;
    int weight = STAR_WEIGHT;
    if (weight_dir == DIAG){
        weight = get_weight(x,y,str_in,input);
    }
    return gains[x][y] + weight;
}


int calc_gain_at(size_t gian_x, size_t gain_y,  str_data* str_in, in_data* input,
                 int            gains[ str_in->len_x + 1 ][ str_in->len_y + 1],
                 direction directions[ str_in->len_x + 1 ][ str_in->len_y + 1] ) {


    //direction DIAG
    int gain3 = calc_gain_candidate( gian_x + 1, gain_y + 1, str_in, input, gains, DIAG);
   
    //direction RIGHT
    int gain1 = calc_gain_candidate( gian_x + 1, gain_y,     str_in, input, gains, RIGHT);

    //direction UP
    int gain2 = calc_gain_candidate( gian_x    , gain_y + 1, str_in, input, gains, UP);

    direction direction;
    gains[gian_x][gain_y] = max(gain1,gain2,gain3, &direction);
    directions[gian_x][gain_y] = direction;
    printf("(%lu, %lu): %d, %d\n", gian_x, gain_y, gains[gian_x][gain_y], direction);
    return 0;
}

//len_x, len_y are respective string lengths, the graph dims are one larger
void calc_gains( str_data * str_in, in_data* input,
                 int gains[str_in->len_x + 1][str_in->len_y + 1],
                 direction directions[ str_in->len_x + 1 ][ str_in->len_y + 1] ) {
    ...
    for(int i = 1; i < min_dim ; i++){
        ...
        for(int j = 0; j < i + 1; j++){
            ...
            calc_gain_at( x_base + j, len_y - j, str_in, input, gains, directions);
...
int main(){
    ...
    scanf("%" STR(MAXLENGTH) "s %" STR(MAXLENGTH) "s", str_in.str_x, str_in.str_y);
    str_in.len_x = strlen(str_in.str_x);
    str_in.len_y = strlen(str_in.str_y);

    int gains[str_in.len_x+1][str_in.len_y+1];
    direction directions[ str_in.len_x + 1 ][ str_in.len_y + 1];
    calc_gains( &str_in, &input , gains, directions );

    int x = 0;
    int y = 0;

    int stars_x[str_in.len_y];
    int stars_y[str_in.len_x];
    ...
    while (x != str_in.len_x && y != str_in.len_y){
        ...
        case RIGHT: stars_y[stars_y_idx++] = x++; break;
        case UP: stars_x[stars_x_idx++] = y++; break;
        case DIAG: x++; y++; break;
        }
